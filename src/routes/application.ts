...
import { createApplication, uploadFiles, getUploadedApplicationsByJobId, updateApplicationStatus, updateApplication, getUploadedApplicationById, getMyUploadedApplicationById } from "../controllers/application";
...
router.post('/upload', passport.authenticate("jwt", { session: false }), checkRole([UserRole.VENDOR]), checkSubscription(), upload.single('file'), expressAsyncHandler(uploadFiles));
